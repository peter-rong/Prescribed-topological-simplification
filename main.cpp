...
int main(int argc, char* argv[]) {
    ...
    if (mode == "cubical" || mode == "cubic") {
        if (subArgc < 3) {
            std::cerr << "Cubical mode needs: <input.mrc> <adjustment>\n\n";
            printUsage(argv[0]);
            return 1;
        }
    ...
    if (mode == "tet") {
        if (subArgc < 4) {
            std::cerr << "Tet mode needs: <mesh.msh> <alpha_file> <adjustment>\n\n";
            printUsage(argv[0]);
            return 1;
        }
